...
# GUI-only settings file. The QuickSync4Linux CLI package keeps its own
# configuration independently, so the GUI can be installed separately.
DEFAULT_CONFIG_FILE = os.path.join(_CONFIG_DIR, 'settings.ini')
...
# Separator + program start, logged only once via a sentinel file
...
def save_settings(path=DEFAULT_CONFIG_FILE):
    try:
        ...
        config_parser = configparser.ConfigParser()
        config_parser.read(path)  # Preserve existing keys, e.g. "device"
        if not config_parser.has_section('general'):
...
# ─── Connection error interpretation ──────────────────────────────────────────
